...
DEFAULT_EXECUTION_ROOT = Path(
    "/Users/davidabiera/Projects/team/autoresearch-macos/worktrees/execution-weight-decay-022-ridge"
)
DEFAULT_STATE_PATH = DEFAULT_EXECUTION_ROOT / "state" / "overnight_execution-weight-decay-022-ridge.json"
DEFAULT_REPORT_PATH = DEFAULT_EXECUTION_ROOT / "reports" / "overnight_execution-weight-decay-022-ridge.md"
DEFAULT_LOG_ROOT = DEFAULT_EXECUTION_ROOT / "logs" / "overnight" / "execution-weight-decay-022-ridge"
BASELINE_IDS = (
    "candidate_weight_decay_022_fixed_step_repeat_m",
    "candidate_weight_decay_022_fixed_step_repeat_n",
)
CANDIDATE_IDS = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Review the mar10 weight-decay ridge fixed-step bracket run.")
...
def _is_clean(item: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    return item.get("val_bpb") is not None, None if item.get("val_bpb") is not None else "missing-val"


def evaluate_weight_decay_ridge_fixed_step_bracket(
    state: dict[str, Any],
    report_path: Path,
    log_root: Path,
) -> dict[str, Any]:
    ...
    if classification is None:
        if best_candidate is None:
            ...
            next_day_action = "close the ridge axis and retain plain `WEIGHT_DECAY=0.22` as the lead"
        elif float(best_candidate["delta"]) > MATERIAL_WIN_THRESHOLD:
            classification = "ridge_promoted"
            next_day_action = f"promote `{best_candidate['id']}` as the single next-day ridge confirmation target"
        elif float(best_candidate["delta"]) > 0.0:
            classification = "ridge_promising"
            next_day_action = f"schedule one bounded confirmation for `{best_candidate['id']}` against the same `0.22` baseline bracket"
        else:
            classification = "ridge_closed"
            next_day_action = "close the ridge axis and retain plain `WEIGHT_DECAY=0.22` as the lead"
    ...
    return {
        ...
        "baseline_m": baseline_values.get(BASELINE_IDS[0]),
        "baseline_n": baseline_values.get(BASELINE_IDS[1]),
...
def render_markdown(review: dict[str, Any]) -> str:
    lines = [
        "# mar10 Weight-Decay Ridge Fixed-Step Bracket Review",
        "",
        "## Artifacts",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- baseline `m`: `{format_float(review['baseline_m'])}`",
            f"- baseline `n`: `{format_float(review['baseline_n'])}`",
    ...
    if not review["candidate_rows"]:
        ...
        for row in review["candidate_rows"]:
            lines.append(
                ...
                f"status=`{row['status']}` status_class=`{row['status_class']}`"
            )
    ...
    lines.extend(
        [
            ...
            f"- best candidate: `{review['best_candidate']['id'] if review['best_candidate'] else 'none'}`",
            "",
...
def main() -> None:
    ...
    if not state:
        raise SystemExit(f"missing state file: {args.state}")

...
DEFAULT_EXECUTION_ROOT = Path(
    "/Users/davidabiera/Projects/team/autoresearch-macos/worktrees/execution-weight-decay-022-unembedding-followup"
)
DEFAULT_STATE_PATH = DEFAULT_EXECUTION_ROOT / "state" / "overnight_execution-weight-decay-022-unembedding-followup.json"
DEFAULT_REPORT_PATH = DEFAULT_EXECUTION_ROOT / "reports" / "overnight_execution-weight-decay-022-unembedding-followup.md"
DEFAULT_LOG_ROOT = DEFAULT_EXECUTION_ROOT / "logs" / "overnight" / "execution-weight-decay-022-unembedding-followup"
BASELINE_IDS = (
    "candidate_weight_decay_022_fixed_step_repeat_k",
    "candidate_weight_decay_022_fixed_step_repeat_l",
)
UNEMBEDDING_IDS = (
    "unembedding_lr_0047_on_weight_decay_022_fixed_step",
    "unembedding_lr_0048_on_weight_decay_022_fixed_step",
)
EXPECTED_IDS = (BASELINE_IDS[0], UNEMBEDDING_IDS[0], UNEMBEDDING_IDS[1], BASELINE_IDS[1])
REQUIRED_STEPS = 354
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Review the mar10 unembedding fixed-step bracket run.")
...
def evaluate_unembedding_fixed_step_bracket(
    state: dict[str, Any],
    report_path: Path,
    log_root: Path,
) -> dict[str, Any]:
    ...
    if not order_matches:
        operational_issues.append("completed item order does not match expected fixed-step bracket order")
    ...
    if baseline_mean is not None:
        for item_id in UNEMBEDDING_IDS:
            item = _require_item(completed_by_id, item_id)
            value = item.get("val_bpb")
            if value is None:
                continue
            ...
            if row["clean"] and (best_candidate is None or float(delta) > float(best_candidate["delta"])):
                best_candidate = row

    if classification is None:
        if best_candidate is None:
            classification = "unembedding_closed"
            next_day_action = "close unembedding and retain plain `WEIGHT_DECAY=0.22` as the lead"
        elif float(best_candidate["delta"]) > MATERIAL_WIN_THRESHOLD:
            classification = "unembedding_promoted"
            next_day_action = f"promote `{best_candidate['id']}` as the single next-day unembedding confirmation target"
        elif float(best_candidate["delta"]) > 0.0:
            classification = "unembedding_promising"
            next_day_action = f"schedule one bounded confirmation for `{best_candidate['id']}` against the same candidate-root baseline"
        else:
            classification = "unembedding_closed"
            next_day_action = "close unembedding and retain plain `WEIGHT_DECAY=0.22` as the lead"
    ...
    return {
        ...
        "baseline_k": baseline_values.get(BASELINE_IDS[0]),
        "baseline_l": baseline_values.get(BASELINE_IDS[1]),
...
def render_markdown(review: dict[str, Any]) -> str:
    lines = [
        "# mar10 Unembedding Fixed-Step Bracket Review",
        "",
        "## Artifacts",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- baseline `k`: `{format_float(review['baseline_k'])}`",
            f"- baseline `l`: `{format_float(review['baseline_l'])}`",
    ...
    if not review["candidate_rows"]:
        lines.append("- none")
    else:
        for row in review["candidate_rows"]:
            lines.append(
                f"- `{row['id']}` `val_bpb={format_float(row['val_bpb'])}` "
                f"`delta={format_float(row['delta'])}` `status={row['status']}` "
                f"`status_class={row['status_class']}` `clean={str(row['clean']).lower()}`"
            )
            if row["log_path"]:
...
def main() -> None:
    ...
    review = evaluate_unembedding_fixed_step_bracket(state, report_path, log_root)
    if args.format == "json":

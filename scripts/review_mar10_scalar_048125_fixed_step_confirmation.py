...
DEFAULT_EXECUTION_ROOT = Path(
    "/Users/davidabiera/Projects/team/autoresearch-macos/worktrees/execution-weight-decay-022-scalar-048125-confirmation"
)
DEFAULT_STATE_PATH = (
    DEFAULT_EXECUTION_ROOT / "state" / "overnight_execution-weight-decay-022-scalar-048125-confirmation.json"
)
DEFAULT_REPORT_PATH = (
    DEFAULT_EXECUTION_ROOT / "reports" / "overnight_execution-weight-decay-022-scalar-048125-confirmation.md"
)
DEFAULT_LOG_ROOT = (
    DEFAULT_EXECUTION_ROOT / "logs" / "overnight" / "execution-weight-decay-022-scalar-048125-confirmation"
)
BASELINE_IDS = (
    "candidate_weight_decay_022_fixed_step_repeat_i",
    "candidate_weight_decay_022_fixed_step_repeat_j",
)
CANDIDATE_IDS = (
    "scalar_lr_048125_fixed_step_confirmation_c",
    "scalar_lr_048125_fixed_step_confirmation_d",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Review the mar10 scalar 0.48125 fixed-step confirmation run.")
...
def evaluate_scalar_048125_fixed_step_confirmation(
    state: dict[str, Any],
    report_path: Path,
    log_root: Path,
) -> dict[str, Any]:
    ...
    if classification is None and delta is not None:
        if delta > MATERIAL_WIN_THRESHOLD:
            ...
            next_day_action = "confirm `SCALAR_LR=0.48125` as the new lead on top of `WEIGHT_DECAY=0.22`"
        elif delta > 0.0:
            classification = "scalar_promising"
            next_day_action = (
                "keep `SCALAR_LR=0.48125` as promising but not confirmed; schedule one bounded follow-up confirmation"
            )
        else:
        ...
        next_day_action = (
            "keep plain `WEIGHT_DECAY=0.22` as the confirmed lead and keep `SCALAR_LR=0.48125` promising but unconfirmed"
        )
    elif classification == "stage_instability":
        next_day_action = "stabilize the fixed-step scalar confirmation stage before judging `SCALAR_LR=0.48125`"
    ...
    return {
        ...
        "baseline_i": baseline_values.get(BASELINE_IDS[0]),
        "baseline_j": baseline_values.get(BASELINE_IDS[1]),
...
def render_markdown(review: dict[str, Any]) -> str:
    lines = [
        "# mar10 Scalar 0.48125 Fixed-Step Confirmation Review",
        "",
        "## Artifacts",
        "",
        ...
        f"- issues: `{'; '.join(review['operational_issues']) if review['operational_issues'] else 'none'}`",
        "",
        "## Baseline Bracket Verdict",
        "",
        f"- baseline `i`: `{review['baseline_i']:.6f}`" if review["baseline_i"] is not None else "- baseline `i`: `missing`",
        f"- baseline `j`: `{review['baseline_j']:.6f}`" if review["baseline_j"] is not None else "- baseline `j`: `missing`",
        f"- `baseline_mean`: `{review['baseline_mean']:.6f}`"
        if review["baseline_mean"] is not None
        else "- `baseline_mean`: `n/a`",
        f"- `baseline_spread`: `{review['baseline_spread']:.6f}`"
        if review["baseline_spread"] is not None
        else "- `baseline_spread`: `n/a`",
        f"- verdict: `{review['baseline_verdict']}`",
        "",
        "## Candidate Repeatability Verdict",
        "",
        f"- candidate `c`: `{review['candidate_c']:.6f}`"
        if review["candidate_c"] is not None
        else "- candidate `c`: `missing`",
        f"- candidate `d`: `{review['candidate_d']:.6f}`"
        if review["candidate_d"] is not None
        else "- candidate `d`: `missing`",
        f"- `candidate_mean`: `{review['candidate_mean']:.6f}`"
        if review["candidate_mean"] is not None
        else "- `candidate_mean`: `n/a`",
        f"- `candidate_spread`: `{review['candidate_spread']:.6f}`"
        if review["candidate_spread"] is not None
        else "- `candidate_spread`: `n/a`",
        f"- `delta`: `{review['delta']:.6f}`" if review["delta"] is not None else "- `delta`: `n/a`",
    ...
    for item in review["item_details"]:
        lines.append(
            f"- `{item['id']}` `status={item['status']}` `val_bpb={item['val_bpb']}` "
            f"`num_steps={item['num_steps']}` `status_class={item['status_class']}` "
            f"`completion_phase={item['completion_phase']}` `training={item['training_seconds']:.6f}` "
            f"`eval={item['eval_seconds']:.6f}` `total={item['total_seconds']:.6f}` "
            f"`max_dt_ms={item['max_step_dt_ms']}` `last_dt_ms={item['last_step_dt_ms']}` "
...
def main() -> None:
    ...
    review = evaluate_scalar_048125_fixed_step_confirmation(state, report_path, log_root)
    if args.format == "json":
